...
async function run() {
  ...
  const allowedPrAuthors = core.getInput("pr_authors").split(",").map(author => author.trim())
  if (!allowedPrAuthors.includes(prAuthor)) {
    console.log(`Author ${prAuthor} is not in expected authors list: ${allowedPrAuthors.join(", ")}, skipping...`)
    return
  }
  ...
  const body = { data: { name: taskName, projects: [asanaProjectId], memberships: [{ project: asanaProjectId, section: asanaSectionId }] } }
  const opts = {}

  console.log(`Creating Asana task for PR #${prNumber} - ${prTitle} by ${prAuthor}`)
